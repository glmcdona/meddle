		...
		try:
			...
			for i in range(len(args)):
				...
				if str(arg) == "-printonly":
					printOnly = True
				elif str(arg) == "-server" and i + 1 < len(args):
					server = args[i+1]
			...
			mProcess = ProcessRdp(self, "C:\\Crash\\", mBreakpoint, -1, 0, printOnly, server )
			...
			while True:
				for j in range(len(report_events_by_target)):
					...
					newProcess = ProcessRdp(self, "C:\\Crash\\", newBreakpoint, -1, unique_identifier, False, server )
